...
import 'package:nims_mobile_app/core/domain/models/facility_type.dart';

import '../../services/remote/nims_api_service.dart';
import '../../services/local/nims_local_service.dart';
...
class FacilitiesRepository {
  ...
  /// Gets only PCR and GeneXpert facilities from cache
  Future<Result<List<DomainFacility>>> getPcrAndGeneXpertFacilities() async {
    try {
      final cachedFacilities = await _localService.getCachedFacilities();
      final filteredFacilities = cachedFacilities.where((facility) {
        final type = facility.type?.toLowerCase();
        return type == FacilityType.pcr.name || type == FacilityType.genexpert.name;
      }).toList();
      return Success(filteredFacilities);
    } catch (e, s) {
      developer.log(
        e.toString(),
        error: e,
        stackTrace: s,
        name: "FacilitiesRepository:getPcrAndGeneXpertFacilities",
      );
