...
class ResultShipmentScreen extends ConsumerStatefulWidget {
  final DomainMovementType movementType;
  final List<LabResult> selectedResults;
  final DomainFacility pickUpFacility;

  const ResultShipmentScreen({
    super.key,
  ...
  @override
  ConsumerState<ResultShipmentScreen> createState() =>
      _ResultShipmentScreenState();
}

class _ResultShipmentScreenState extends ConsumerState<ResultShipmentScreen> {
  ...
  void initState() {
    super.initState();
    _providerKey =
        resultShipmentsScreenKey(widget.movementType, widget.pickUpFacility);
    // Initialize the state after the first frame
    WidgetsBinding.instance.addPostFrameCallback((_) {
      ref
          .read(resultShipmentsScreenStateNotifierProvider(_providerKey).notifier)
          .initialize(
            movementType: widget.movementType,
            pickUpFacility: widget.pickUpFacility,
            selectedResults: widget.selectedResults,
          );
    });
  }
  ...
  Widget build(BuildContext context) {
    ...
    final asyncValueState = ref.watch(
      resultShipmentsScreenStateNotifierProvider(_providerKey),
    );
    ...
    return asyncValueState.when(
      data: (state) => PopScope(
        ...
        onPopInvokedWithResult: (didPop, result) {
          if (didPop) {
            ...
            ref.read(resultShipmentsScreenStateNotifierProvider(_providerKey).notifier).clearCache();
          }
        },
                        ...
                        children: [
                          ...
                          DropdownMenu<String>(
                            ...
                            onSelected: (value) {
                              ref
                                  .read(
                                    resultShipmentsScreenStateNotifierProvider(
                                      _providerKey,
                                    ).notifier,
                                  )
                            ...
                            return Padding(
                              ...
                              child: NIMSShipmentCard(
                                ...
                                onTap: () {
                                  ...
                                  final currentState = ref.read(resultShipmentsScreenStateNotifierProvider(_providerKey)).value;
                                  final latestShipment = currentState?.shipments.firstWhere(
                                    (s) => s.shipmentNo == shipment.shipmentNo,
                                    orElse: () => shipment,
                                  ) ?? shipment;
                                  // Get the sample codes for this shipment
                                  context.pushNamed(
                                    shipmentDetailsScreen,
                                    queryParameters: {
                                      shipmentQueryParam:
                                          jsonEncode(latestShipment.toJson()),
                                      shipmentSampleCodesQueryParam:
                                          jsonEncode(currentState?.shipmentSampleCodes),
                                    },
                                  );
                                },
                                onSelectDestinationLocationType: null
              ...
              context.pushNamed(
                ...
                queryParameters: {
                  ...
                  shipmentsQueryParam: jsonEncode(
                    state.shipments.map((s) => s.toJson()).toList(),
                  ),
                  sampleCodesQueryParam: jsonEncode(state.shipmentSampleCodes),
                },
              );
            },
      ...
      error: (e, s) => NIMSErrorContent(
        ...
        onTapActionButton: () {
          ref
              .read(
                  resultShipmentsScreenStateNotifierProvider(_providerKey).notifier)
              .refreshState();
        },
