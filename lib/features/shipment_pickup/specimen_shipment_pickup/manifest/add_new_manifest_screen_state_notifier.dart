...
import 'add_new_manifest_screen_state.dart';
...
class AddNewManifestScreenStateNotifier
  ...
  Future<AddNewManifestScreenState> _loadData(
    ...
    final facilitiesResult = await facilitiesRepository.getPcrAndGeneXpertFacilities();
    ...
    if (data == null) {
      ...
      final result = await manifestRepository.saveManifest(
        DomainManifest(
          ...
          destinationFacilityName:
              data.selectedDestinationFacility?.facilityName ?? "",
        ),
        data.samples,
      );
      ...
      switch (result) {
        case Success<bool>():
          ...
          state = state.whenData(
            (data) => data.copyWith(isSavingManifest: false),
          );
          if (state.requireValue.movementType != null) {
            ref
                .read(
                  selectManifestsScreenStateNotifierProvider((
                    movementType: state.requireValue.movementType!,
                  )).notifier,
                )
                .onSaveManifest();
          }
